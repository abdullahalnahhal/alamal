...
@section('title', trans('admin/sidebar.Products'))
@push('css')
	    ...
	    <div class="row" >
	    	<div class="col-12" >
	    		<div class="card" >
	    			<div class="card-body">
	    				<h4 class="card-title">
		    				<span class="text-danger pull-{{getStyle(App::getLocale())['dir-full']}}">@lang('admin/products.Add Product')</span>
		    			...
		    			<div class="row">
			    			<div class="col-8">
			    				<form method="post" action="{{lRoute('admin.products.add')}}" id="add-section-form">
			    					@csrf
			    					<input type="hidden" id='completed' name="completed" value="0">
                        			<input type="hidden" id='file-name' name="file-name" value="">
				    				<div class="col-12 row p-0">
					    				<div class="col-4 pt-2 "><b>@lang('admin/products.Partner Name')</b></div>
					    				<div class="col-8 pr-0 mr-0">
					    					<select class="form-control" name="partner" required>
					    						<option>__ @lang('admin/products.Partner Name') __</option>
					    						@foreach($partners as $partner)
					    							<option value="{{$partner->id}}">{{$partner->{'title_'.App::getLocale()} }}</option>
					    						@endforeach
					    					</select>
					    				</div>
					    			</div>
					    			<div class="vtabs customvtab  col-12">
			                            ...
			                            <div class="tab-content">
			                            	<div id="sec-en" class="tab-pane active">
								    			<div class="col-12 row p-0">
								    				<div class="col-4 pt-2 "><b>@lang('admin/products.Product Name')</b></div>
								    			...
								    			<div class="col-12 row p-0" >
								    				<div class="col-4 pt-2 "><b>@lang('admin/products.Product Description')</b></div>
								    				<div class="col-8 pr-0 mr-0">
	    	...
	    	@foreach($products as $product)
		        <div class="col-4" >
		            <div class="card">
		                <div class="card-body">
		                    <img src="{{asset("slider")}}/{{$product->file_name}}" style="max-height: 180px;max-width:100%;">
		                </div>
		            </div>
		        </div>
		        <div class="col-4">
		            <div class="card">
		                <div class="card-body">
		                    ...
		                    <form method="post" id="form-update-{{$product->id}}" action="{{lRoute('admin.products.update',['id'=>$product->id])}}">
		                    	@csrf
		                    	<div class="form-group">
					    			<select class="form-control" name="partner" required>
					    				<option>__ @lang('admin/products.Partner Name') __</option>
					    				@foreach($partners as $partner)
					    					<option value="{{$partner->id}}" {{($product->partner_id == $partner->id)?"selected":""}} {{($partner->id == $product->partner_id)?"selected":""}} >{{$partner->{'title_'.App::getLocale()} }}</option>
					    				@endforeach
					    			</select>
					    		</div>
					    		<ul class="nav nav-tabs" role="tablist">
				                    <li class="nav-item"> <a class="nav-link active" data-toggle="tab" href="#product-en-{{$product->id}}" role="tab"><span class="hidden-sm-up"><i class="ti-home"></i></span> <span class="hidden-xs-down">@lang('admin/all.en')</span></a> </li>
				                    <li class="nav-item"> <a class="nav-link" data-toggle="tab" href="#product-ar-{{$product->id}}" role="tab"><span class="hidden-sm-up"><i class="ti-user"></i></span> <span class="hidden-xs-down">@lang('admin/all.ar')</span></a> </li>
				                </ul>
				                <div class="tab-content tabcontent-border">
				                	<div id="product-en-{{$product->id}}" class="tab-pane active">
				                    	<input type="text" name="title-en" class="form-control" placeholder="title" value="{{$product->title_en}}">
				                        <textarea class="form-control text-{{getStyle(App::getLocale())['dir-full']}}" name='content-en' id="description" rows="2" placeholder="Enter text ..." style="height:95px;text-align: left;">{{$product->description_en}}</textarea>
		                    		</div>
				                	<div id="product-ar-{{$product->id}}" class="tab-pane">
				                    	<input type="text" name="title-ar" class="form-control" placeholder="title" value="{{$product->title_ar}}">
				                        <textarea class="form-control text-{{getStyle(App::getLocale())['dir-full']}}" name='content-ar' id="description" rows="2" placeholder="Enter text ..." style="height:95px;text-align: left;">{{$product->description_ar}}</textarea>
				                    </div>
				                </div>
		        ...
		        <div class="col-4">
		            <div class="card">
		                <div class="card-body">
		                	...
		                	<br>
		                	<br>
		                	<a href="{{lRoute('admin.products.delete', ['id'=>$product->id])}}" class="btn btn-info  btn-block btn-addon">
		                		<i class="ti-close" ></i> @lang("admin/slider.Delete")
		                	</a>
		                    <button num='{{$product->id}}' class="btn btn-block btn-addon btn-danger update ">
